...
class NotificationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () =>
          _handleTap(context, notification.type ?? 'general', metadata),
      child: Container(
                  ...
                  Container(
                    ...
                    child: Center(
                      child: Icon(
                        ...
                        color: _iconColor(
                            notification.type ?? 'general', colorScheme),
                        size: 20.sp,
                      ),
                    ),
                  ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4.h),
                    Text(
                      _translateMessage(context, notification.message ?? ''),
  ...
  String _translateTitle(BuildContext context, String title) {
    ...
    if (lowerTitle == 'parcel created') {
      return l10n?.parcelCreated ?? 'Parcel Created';
    }
    if (lowerTitle == 'parcel status updated' ||
        lowerTitle == 'order status updated') {
      return l10n?.parcelStatusUpdated ?? 'Parcel Status Updated';
    }
    return title;
  }
  ...
  String _translateMessage(BuildContext context, String message) {
    ...
    // Pattern: Parcel PAR-XXX created
    final parcelCreatedRegex =
        RegExp(r'Parcel (PAR-[\w-]+) created', caseSensitive: false);
    if (parcelCreatedRegex.hasMatch(message)) {
      final match = parcelCreatedRegex.firstMatch(message);
      final parcelId = match?.group(1) ?? '';
      return l10n?.parcelCreatedMessage(parcelId) ?? message;
    }

    // Pattern: Parcel PAR-XXX updated to [status]
    final parcelUpdatedRegex = RegExp(
        r'Parcel (PAR-[\w-]+) updated to ([\w\s]+)',
        caseSensitive: false);
    if (parcelUpdatedRegex.hasMatch(message)) {
      final match = parcelUpdatedRegex.firstMatch(message);
      final parcelId = match?.group(1) ?? '';
      final status = match?.group(2) ?? '';
      final translatedStatus =
          AppHelpers.translateParcelStatus(context, status);
      return l10n?.parcelStatusUpdatedMessage(parcelId, translatedStatus) ??
          message;
    }
  ...
  void _handleTap(BuildContext context, String simplifiedType,
    ...
    context.read<NotificationBloc>().add(MarkAsReadSpecificNotification(
