...
class _ParcelRequestScreenState extends State<ParcelRequestScreen> {
  ...
  void calculateDeliveryCharge(dynamic storedLocation) {
    double sLat = (data["senderLat"] as num?)?.toDouble() ?? (storedLocation?.latitude ?? 0.0);
    double sLng = (data["senderLng"] as num?)?.toDouble() ?? (storedLocation?.longitude ?? 0.0);
    ...
    if (sLat != 0 &&
      ...
      if (data["intercity_charge"] != null) {
        totalDeliveryCharge =
            double.tryParse(data["intercity_charge"].toString()) ?? 0.0;
      } else {
      ...
      if (mounted) {
        context.read<ParcelCreateBloc>().add(
              AddParcelRequest(
                ...
                pbType: data["pb_type"] ?? "Package",
                // Sender details from data map and Global
                sdName: data["senderName"] ?? "",
                sdEmail: Global.userData?.email ?? "",
                sdMobile: "+229${data["senderPhone"] ?? ""}",
                sdAddress: data["senderAddress"] ?? "",
                sdLat: (data["senderLat"] as num?)?.toDouble() ?? 0.0,
                sdLong: (data["senderLng"] as num?)?.toDouble() ?? 0.0,
                sdStreet: data["senderStreet"] ?? "",
                sdHouse: data["senderHouse"] ?? "",
                sdFloor: data["senderFloor"] ?? "",
                // Receiver details from controllers and data map
                rcName: _receiverNameController.text,
                rcMobile: "+229${_receiverMobileController.text}",
                rcAddress: _receiverAddressController.text,
                rcLat: receiverLat ?? 0.0,
                rcLong: receiverLng ?? 0.0,
                rcStreet: _streetController.text,
                rcHouse: _houseController.text,
                rcFloor: _floorController.text,
                // Delivery type logic
                deliveryType:
                    data["intercity_id"] != null ? "Intercity" : "Standard",
                intercityId: data["intercity_id"] as int?,
                pbStatus: "Pending",
                pbWhoPay: payer,
                pbPayMethod: paymentMethod,
                pbDeliveryCharge: totalDeliveryCharge,
              ),
            );
      }
    } else {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              l10n?.parcelRequest ?? "Parcel Request",
              style: Theme.of(context).textTheme.titleMedium?.copyWith(
                    fontSize: 18,
            ...
            Row(
              ...
              children: [
                Text(
                  l10n?.parcelType ?? "Parcel Type",
                  style: const TextStyle(color: Colors.grey),
                ),
                Text(
                  AppHelpers.translateDeliveryType(context, deliveryType),
            ...
            Row(
              ...
              children: [
                Text(
                  l10n?.parcelCategory ?? "Parcel Category",
                  style: const TextStyle(color: Colors.grey),
                ),
                Text(
                  AppHelpers.translateParcelType(context, data["pb_type"]),
                  style: const TextStyle(
                    fontWeight: FontWeight.w600,
                    color: AppTheme.primaryColor,
                  ),
