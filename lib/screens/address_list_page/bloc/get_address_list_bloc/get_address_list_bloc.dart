...
class GetAddressListBloc
    extends Bloc<GetAddressListEvent, GetAddressListState> {
  ...
  Future<void> _onFetchUserAddressList(
      FetchUserAddressList event, Emitter<GetAddressListState> emit) async {
    ...
    try {
      ...
      addressList = List<AddressListData>.from(response['data']['data']
          .map((data) => AddressListData.fromJson(data)));
      print('Fetched ${addressList} addresses');
      currentPage += 1;
      _hasReachedMax = addressList.length < perPage;
      final bool isFetchingAllAddresses =
          event.deliveryZoneId == null || event.deliveryZoneId.toString() == '';
      if (response['success'] == true) {
        ...
        if (!HiveSelectedAddressHelper.hasSelectedAddress() &&
  ...
  Future<void> _onRemoveAddressLocally(
      RemoveAddressLocally event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListLoaded) {
      ...
      final updatedAddressList = currentState.addressList
  ...
  Future<void> _onAddAddressRequest(
      AddAddressRequest event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListLoaded) {
      final currentState = state as GetAddressListLoaded;
      ...
      try {
        ...
        print('Add Address Response@@@@@: $response');
        final addressId = response['data']['id'];
        print("Fetched Address ID from response: $addressId");
        await SecureStorageHelper.saveSelectedAddressId(addressId);
        print("Address ID stored in SecureStorage: $addressId");
        if (response.isNotEmpty) {
          // add(FetchUserAddressList(deliveryZoneId: event.deliveryZoneId));
          add(FetchUserAddressList());
  ...
  Future<void> _onUpdateAddressRequest(
      UpdateAddressRequest event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListLoaded) {
      final currentState = state as GetAddressListLoaded;
      ...
      try {
        ...
        if (response['success'] == true && response['data'] != null) {
          final updatedAddress = AddressListData.fromJson(response['data']);
          ...
          await Future.delayed(const Duration(seconds: 1));
          if (state is GetAddressListLoaded) {
            emit((state as GetAddressListLoaded).copyWith(isUpdated: false));
          }
        } else {
  ...
  Future<void> _onRemoveAddressRequest(
      RemoveAddressRequest event, Emitter<GetAddressListState> emit) async {
    if (state is GetAddressListLoaded) {
      final currentState = state as GetAddressListLoaded;
      emit(currentState.copyWith(isRemoving: true));
      ...
      try {
        ...
        if (response['success'] == true) {
          ...
          if (updatedAddressList.isEmpty) {
            HiveSelectedAddressHelper.clearSelectedAddress();
          } else if (HiveSelectedAddressHelper.getSelectedAddress()?.id ==
              event.addressId) {
            HiveSelectedAddressHelper.clearSelectedAddress();
          }
          ...
          emit(GetAddressListFailed(
              error: response['message'] ?? 'Failed to remove address'));
        }
      } catch (e) {
