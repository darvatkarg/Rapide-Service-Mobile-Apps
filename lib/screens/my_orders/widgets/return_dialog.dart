...
class _ReturnItemsDialogState extends State<ReturnItemsDialog> {
    ...
    return Dialog(
      ...
      child: Column(
        ...
        children: [
          _header(context),
          Expanded(
            child: _selectedItem == null ? _buildListView() : returnForm(),
          ),
          if (_selectedItem != null) _footerActions()
        ],
      ),
    );
  }
  ...
  Widget _header(BuildContext context) {
    ...
    final title =
        _selectedItem == null ? l10n.myOrders : l10n.cancelReturnRequest;
    return Container(
  ...
  Future<void> _pickImages() async {
    ...
    if (remaining <= 0) {
      ToastManager.show(
        context: context,
        message:
            AppLocalizations.of(context)!.youCanUploadUpToImagesOnly(maxImages),
        ...
        if (mounted) {
          ToastManager.show(
            ...
            message: AppLocalizations.of(context)!
                .onlyMoreImagesAddedMaxLimit(remaining, maxImages),
            type: ToastType.info,
          );
        }
      }
  ...
  Future<void> _confirmCancelItem(OrderItems item) async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content:
              Text(AppLocalizations.of(ctx)!.areYouSureYouWantToCancelThisItem),
          actions: [
            TextButton(
    ...
    if (confirmed == true) {
      if (mounted) {
        context
            .read<ReturnOrderItemBloc>()
            .add(CancelOrderItem(orderItemId: item.id!));
  ...
  Future<void> _confirmCancelReturnRequest(OrderItems item) async {
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content: Text(AppLocalizations.of(ctx)!
              .areYouSureYouWantToCancelThisReturnRequest),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(ctx).pop(false),
              child: Text(
    ...
    if (confirmed == true) {
      if (mounted) {
        context
            .read<ReturnOrderItemBloc>()
            .add(CancelReturnRequest(orderItemId: item.id!));
        Navigator.pop(context);
      }
    }
  }
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 4.h),
              Text('${AppLocalizations.of(context)!.quantity}: $qty',
                  style: TextStyle(fontSize: 12.sp)),
              Text(
                  '${AppLocalizations.of(context)!.price}: ${AppHelpers.currency}$price',
                  style: TextStyle(fontSize: 12.sp)),
            ],
          ),
        ),
    ...
    return Column(
      ...
      children: [
        ...
        Builder(
          builder: (context) {
            final l10n = AppLocalizations.of(context)!;
            return Text(l10n.reasonForReturn,
  ...
  Widget _buildEmptyState() {
    final l10n = AppLocalizations.of(context); // ← fix: define l10n here
    return InkWell(
      onTap: _pickImages,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              l10n?.tapToUploadPhotos ?? 'Tap to upload photos',
              style: TextStyle(
                fontSize: 13.sp,
            ...
            SizedBox(height: 4.h),
            Text(
              l10n?.helpUsUnderstandExperience ??
          ...
          Expanded(
            child: CustomButton(
              ...
              child: _submitting
                  ...
                  : Text(AppLocalizations.of(context)!.submitReturn,
...
class _ProductReturnCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final status = getItemStatus(context, item);
      ...
      child: Column(
        ...
        children: [
          ...
          if (item.status != 'delivered' && item.product!.isCancelable == false)
            _warningText(
                AppLocalizations.of(context)!.thisProductIsNotCancelable,
                context)
          else if (item.product!.isReturnable == false)
            _warningText(
                AppLocalizations.of(context)!.thisProductIsNotReturnable,
                context),

          // 2. Dynamic status message (cancelled, return in progress, refunded, etc.)
          if (status.message != null) ...[
            if (item.product!.isCancelable == false ||
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  child:
                      Text(AppLocalizations.of(context)!.cancelReturnRequest),
                ),
              ),
            ),
        ],
  ...
  bool _canCancelReturnRequest() {
    ...
    return r.returnStatus == 'requested' ||
        (r.returnStatus != 'seller_approved' &&
            r.returnStatus != 'cancelled' &&
        ...
        child: Row(
          children: [
            Icon(Icons.info_outline,
                size: isTablet(context) ? 10.sp : 16.sp,
  ...
  Widget _statusMessage(ItemStatus status, BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(status.icon,
              size: isTablet(context) ? 11.sp : 18.sp, color: status.color),
          SizedBox(width: 8.w),
          Expanded(
            child: Text(
              status.message!,
    ...
    return Column(
      ...
      children: [
        ...
        Text('${AppLocalizations.of(context)!.qty}: $qty',
            style: TextStyle(fontSize: isTablet(context) ? 16 : 10.sp)),
        Text('${AppHelpers.currency}$price',
            style: TextStyle(fontSize: isTablet(context) ? 16 : 10.sp)),
      ],
    );
  }
