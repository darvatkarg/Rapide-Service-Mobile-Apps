...
//   List<BannerModel> getBanners(BuildContext context) {
...
//   List<_SendCategory> getCategories(BuildContext context) {
//     final l10n = AppLocalizations.of(context);

//     return [
//       _SendCategory(
//         imagePath: 'assets/images/gift.png',
//         title: l10n?.gift ?? "Gift",
//         subtitle:
//             l10n?.sendHeartfelt ?? "Send heartfelt presents, right on time",
//       ),
//       _SendCategory(
//         imagePath: 'assets/images/document.png',
//         title: l10n?.document ?? "Document",
//         subtitle: l10n?.fromIDs ?? "From IDs to forms, we deliver it all",
//       ),
//       _SendCategory(
//         imagePath: 'assets/images/electronics.png',
//         title: l10n?.electronic ?? "Electronic",
//         subtitle:
//             l10n?.safeguard ?? "Safeguard your gadgets with secure delivery",
//       ),
//       _SendCategory(
...
//     final banners = getBanners(context);
...
//                           Row(
//                             children: [
//                               Expanded(child: _buildTopAddress()),
...
//                                   builder: (context, state) {
//                                     final unreadCount = SettingsData
//                                             .instance
//                                             .notification
...
//                             child: Text(
//                               l10n?.whtu ?? 'What would you like to send?',
//                               style: TextStyle(
//                                 fontSize: 16.sp,
...
//                                   backgroundColor: Colors.red,
//                                   child: Icon(
//                                     HeroiconsSolid.bell, color: textColor),
//                                 );
//                               },
//                             )),
//                           SizedBox(width: 10,),
...
// class _SendCategoryCard extends StatelessWidget {
...
//           // Location not selected → open bottomsheet
//           _openLocationBottomSheet(context);
//         } else {
//           // Location available → navigate
//           GoRouter.of(context).push(
//             AppRoutes.parcelDetail,
//             extra: category.title,
//           );
...
//           mainAxisAlignment: MainAxisAlignment.center,
//           children: [
//             Image.asset(category.imagePath, width: 60.w, height: 60.h),
//             // SizedBox(height: 5.h),
//             Text(
//               category.title,
//               style: TextStyle(
...
class HomePage extends StatefulWidget {
  final int? resetId;
  const HomePage({super.key, this.resetId});
...
class _HomePageState extends State<HomePage>
  ...
  bool _isLoadingIntercities = false;
  ...
  void initState() {
    ...
    _startAutoScroll();
    _fetchIntercities();
    _lastResetId = widget.resetId;
  }

  @override
  void didUpdateWidget(HomePage oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.resetId != null && widget.resetId != _lastResetId) {
      _lastResetId = widget.resetId;
      _resetHomeState();
    }
  }
  ...
  Future<void> _fetchIntercities() async {
    if (mounted) {
      setState(() {
        _isLoadingIntercities = true;
      });
      try {
        final response = await AppHelpers.apiBaseHelper.getAPICall(
          ApiRoutes.intercitiesApi,
          {},
          isUserApi: false,
        );

        if (response.statusCode == 200 && response.data['success'] == true) {
          final List<dynamic> intercities =
              response.data['data']['intercities'];
          setState(() {
            _interCityRoutes =
                intercities.map((e) => e as Map<String, dynamic>).toList();
          });
        }
      } catch (e) {
        print("Error fetching intercities: $e");
      } finally {
        setState(() {
          _isLoadingIntercities = false;
        });
      }
    }
  }
  ...
  List<BannerModel> getBanners(BuildContext context) {
    return [
      BannerModel(imageUrl: 'assets/images/banner_1.png'),
      BannerModel(imageUrl: 'assets/images/banner_2.png'),
    ];
  }
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          children: [
                            ...
                            Row(
                              children: [
                                if (Global.userData != null)
                                  Expanded(
                                    child: Text(
                                      '${l10n?.hiGreeting ?? "Hi !"} ${Global.userData?.name ?? ''}',
                                      style: TextStyle(
                                        fontSize: 16.sp,
                                        fontWeight: FontWeight.bold,
                            ...
                            const SizedBox(height: 10), // Reduced from 30 to 10
                            AnimatedSwitcher(
                              duration: const Duration(milliseconds: 500),
                              transitionBuilder:
                              ...
                              child: _homeViewIndex == 0
                                  ? _buildQuickActionCards()
                                  : _homeViewIndex == 1
                                      ? _buildLivreurOptionsView()
                                      : _buildInterCityOptionsView(),
                            ),
  ...
  Widget _buildQuickActionCards() {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
          color: Colors
              .transparent, // Keeping cards individual look but within a stable frame
        ),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          // Placeholder space to match the title height in the other view
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          l10n?.orderNearestDelivery ??
                              'Order the nearest delivery person to you',
                          textAlign: TextAlign.center,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          l10n?.deliveryCityToCity ??
                              'Delivery from one city to another city',
                          textAlign: TextAlign.center,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
  ...
  Widget _buildInterCityOptionsView() {
    final l10n = AppLocalizations.of(context);
    return Container(
      key: const ValueKey('InterCityActions'),
      decoration: BoxDecoration(
          ...
          Container(
            ...
            child: DropdownButtonFormField<Map<String, dynamic>>(
              ...
              decoration: InputDecoration(
                labelText: l10n?.selectRoute ?? 'Select Route',
                labelStyle:
                ...
                prefixIcon: _isLoadingIntercities
                    ? const Padding(
                    ...
                    : Icon(Icons.route_rounded, color: AppTheme.primaryColor),
              ),
              hint: _isLoadingIntercities
                  ? Text(l10n?.loadingRoutes ?? "Loading routes...")
                  : null,
              items: _interCityRoutes.map((route) {
                return DropdownMenuItem<Map<String, dynamic>>(
                  value: route,
                  child: Text(
                    '${route['from_city']} - ${route['to_city']} ( ${AppHelpers.currency} ${route['charge']} )',
                    style: TextStyle(
                      fontSize: 14.sp,
                      fontWeight: FontWeight.w500,
                      color: Theme.of(context).colorScheme.tertiary,
              ...
              onChanged: (value) {
                setState(() {
                  ...
                  _city1Name = value?['from_city'];
                  _city2Name = value?['to_city'];
                });
              },
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                ...
                elevation: (_city1Name != null && _city2Name != null) ? 2 : 0,
              ),
              onPressed: (_city1Name != null && _city2Name != null)
                  ? () {
                      setState(() {
                        _homeViewIndex = 1;
                      });
  ...
  Widget _buildLivreurOptionsView() {
    final categories = getCategories(context);
    final l10n = AppLocalizations.of(context);

    return Container(
      key: const ValueKey('LivreurActions'),
      // Removing shadow/decoration here to match the 2-card layout better if they are not in a shared container
      // OR, keeping it but ensuring padding/alignment matches perfectly.
      decoration: BoxDecoration(
        color: Colors.transparent,
      ),
      child: Column(
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              return _SendCategoryCard(
                category: categories[index],
                intercityId: _selectedRoute?['id'],
                intercityCharge: _selectedRoute?['charge'],
              );
            },
          ),
        ],
...
// ─────────────────────────────────────────────────────────────────────────────
// Send-category card  (overflow fix applied here)
// ─────────────────────────────────────────────────────────────────────────────

class _SendCategoryCard extends StatelessWidget {
  final _SendCategory category;
  final int? intercityId;
  final dynamic intercityCharge;
  const _SendCategoryCard(
      {required this.category, this.intercityId, this.intercityCharge});
      ...
      onTap: () {
        GoRouter.of(context).push(
          ...
          extra: {
            ...
            'pb_type': category.pbType,
            'intercity_id': intercityId,
            'charge': intercityCharge
          },
        );
      },
      child: Container(
        ...
        // ── FIX: LayoutBuilder lets the Column know its exact constraints,
        //    so we can size the image proportionally and avoid overflow.
        child: LayoutBuilder(
          builder: (context, constraints) {
            final double availableHeight = constraints.maxHeight;
            // Reserve ~40 % for the image, rest for text.
            final double imageSize = availableHeight * 0.38;
