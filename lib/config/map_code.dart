...
class _LocationPickerWidgetState extends State<LocationPickerWidget>
  ...
  LatLng _centerLocation = LatLng(
      double.parse(AppHelpers.defaultLat), double.parse(AppHelpers.defaultLng));
  ...
  Future<void> _selectSuggestion(LocationSuggestion suggestion) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['status'] == 'OK') {
        ...
        final formatted =
            response.data['result']['formatted_address'] as String? ??
                suggestion.address;
            ...
            onTap: () async {
              ...
              if (mapController != null) {
                if (mounted) {
                  ...
                  final screenCoordinate = await mapController!
                      .getScreenCoordinate(LatLng(lat, lng));
  ...
  void _confirmLocation() async {
    final result = {
      ...
      'street': _lastPlacemark?.thoroughfare ?? _lastPlacemark?.street ?? '',
      'house': _lastPlacemark?.subThoroughfare ?? '',
    };

    /// CASE 1 → From Address Screen
    if (widget.isFromAddressPage) {
      ...
      _toggleForm(); // only for address screen
      return;
    }

    /// CASE 2 → From Parcel Screen
    if (mounted) {
                ...
                if (_selectedStore != null && _cardOffset != null && !_isMoving)
                  Builder(
                    builder: (context) {
                      ...
                      left = math.max(
                          16.0, math.min(left, screenWidth - cardWidth - 16.0));
