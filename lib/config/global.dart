...
class Global {
  ...
  static String get currentLanguage {
    // 1. Try to get from user data (if logged in)
    if (_userData?.language != null) {
      return _userData!.language;
    }
    // 2. Fallback to AppPrefsBox (for guest users or if not in user data)
    try {
      final Box prefsBox = Hive.box(_prefsBoxName);
      return prefsBox.get('selectedLanguage', defaultValue: 'fr') as String;
    } catch (e) {
      return 'fr';
    }
  }

  /// Update language inside UserDataModel and AppPrefsBox
  static Future<void> setLanguage(String languageCode) async {
    // 1. Update AppPrefsBox (for all users)
    try {
      final Box prefsBox = await Hive.openBox(_prefsBoxName);
      await prefsBox.put('selectedLanguage', languageCode);
    } catch (e) {
      debugPrint('❌ Failed to save language to prefs: $e');
    }

    // 2. Update UserDataModel (if logged in)
    if (_userData != null) {
