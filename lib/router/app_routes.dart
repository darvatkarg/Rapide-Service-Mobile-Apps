    ...
    routes: [
      ...
      GoRoute(
        ...
        pageBuilder: (context, state) {
          ...
          return platformPage(
            DeliveryZoneMap(
              targetLat: data['target_lat']?.toString() ?? '0.0',
              targetLong: data['target_long']?.toString() ?? '0.0',
              targetName: data['target_name']?.toString() ?? 'Unknown',
              targetAddress: data['target_address']?.toString() ?? 'No Address',
              targetMobile: data['target_mobile']?.toString() ?? '',
              targetType: data['target_type']?.toString() ?? 'Receiver',
            ),
          );
        },
      ),
