...
import 'package:hyper_local/services/translation/language_provider.dart';
...
void main() async {
  ...
  await Future.wait([
    ...
    _initializeHiveAndGlobal(),
    LocalizationService().initialize(),
  ]);

  // Non-critical services can start without blocking the first frame
...
Future<void> _initializeHiveAndGlobal() async {
  await Hive.initFlutter();
  await Future.wait([
    Hive.openBox('theme_box'),
    Global.initializeToken(),
    Global.initializeStatus(),
  ]);
}

void _initializeBackgroundServices() {
  // These are started without 'await' to not block the UI rendering
...
Future<void> createNotificationChannel() async {
  const AndroidNotificationChannel channel = AndroidNotificationChannel(
    ...
    'Location Tracking',
    description: 'Foreground service for delivery tracking',
    ...
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(
          create:
              (context) =>
                  HomeStatsBloc(HomeStatsRepo())..add(FetchHomeStats()),
        ),
